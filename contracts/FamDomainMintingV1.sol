...
contract FamDomainMintingV1 is Initializable, OwnableUpgradeable {
    ...
    // uint256 public constant DOMAIN_MINT_FEE = 5 * 10**18; // 5 USDT token
    ...
    function batchAddDiscountMintWhitelist(
        address[] calldata _users
    ) external onlyOwner {
        for (uint256 i = 0; i < _users.length; i++) {
    ...
    function discountMintDomain(
        ...
        // require(msg.value >= _amount / 2, "Insufficient ETH sent");

        // Transfer 50% of the minting fee
        // uint256 discountFee = _amount / 2;

        // payable(feeRecipient).transfer(discountFee);
